...
/// Самая старая запись — это база отсчёта, "1.0.0"; дальше версия каждой следующей записи
/// считается от версии предыдущей по этому правилу.
///
/// Поле "date" необязательное — старые записи (уже выпущенные версии) им заполнены и остаются
/// с датой навсегда, как и были; НОВЫЕ записи (см. AssignComputedFields) дату уже не указывают —
/// changelog теперь привязан к версии, а не к дате, поэтому и в UI, и в файле дата для будущих
/// версий просто не нужна. Из-за этого сортировка "по хронологии" (см. ChangelogEntryViewModel.
/// ParsedVersion и ChangelogWindow.RefreshVisible) считается по НОМЕРУ ВЕРСИИ, а не напрямую по
/// дате — для старых записей это буквально то же самое, что сортировка по дате (версии и так
/// растут строго по хронологии дат, см. ниже), а для новых, у которых даты вообще нет, только
/// так порядок и можно определить.
///
/// Порядок вычисления версий: сначала все записи С датой — по датам, от самой старой к самой
/// новой (порядок в самом файле для них по-прежнему значения не имеет); затем все записи БЕЗ
/// даты — в том порядке, в котором они перечислены в файле (для них порядок в файле как раз и
/// является единственным источником истины о том, что было раньше, а что позже — раз даты нет).
/// "Текущая версия" — всегда самая последняя в этом полном порядке: последняя по дате, если
/// записей без даты ещё не появилось, либо последняя из недатированных, если уже появились.
///
/// Формат — простой JSON-массив. У каждой строки изменения — свой тип ("added" / "changed" /
/// "fixed" / "removed", см. ChangeTypeCatalog); неизвестный или отсутствующий type трактуется
/// как "changed". И у версии, и у отдельного пункта изменения есть необязательное поле "image" —
/// либо прямая ссылка (http/https), либо имя файла (или относительный путь) внутри этой же папки
/// Changelog, рядом с changelog.json (см. ChangelogImageResolver.Resolve):
/// [
///   {
///     "date": "12 июля 2026",
...
public static class ChangelogLoader
{
    ...
    //  - IsCurrent: у самой последней записи в общем порядке (см. ниже) — не обязательно
    //    последней по счёту в файле.
    //
    // Порядок, по которому версии растут одна из другой, — два куска подряд:
    //   1) записи С заполненным "date" — по датам, от самой старой к самой новой (порядок в
    //      самом файле для них не важен, как и раньше). Записи с нераспознанной датой
    //      (ChangelogDateParser возвращает DateTime.MinValue — например, "Первый релиз") в
    //      этой сортировке естественно оказываются самыми старыми.
    //   2) записи БЕЗ "date" (новый, "версия без даты" формат) — в том порядке, в котором они
    //      просто перечислены в файле. Раз даты нет, единственный способ понять, что из них
    //      было раньше, а что позже, — порядок в самом файле; и весь этот кусок целиком идёт
    //      ПОСЛЕ всех датированных записей, потому что недатированная запись по построению —
    //      это будущая (ещё не date-разбитая) версия.
    // Благодаря этому у уже существующих (датированных) записей номер версии не меняется вообще
    // никак по сравнению с тем, что было раньше, — они по-прежнему обрабатываются в точности в
    // том же порядке, что и раньше; новые (недатированные) записи просто продолжают ту же
