...
// Читает список версий из changelog.json — встроен в сборку как EmbeddedResource (Lumisense.csproj),
// не лежит рядом с .exe отдельным файлом, чтобы его нельзя было поправить прямо из папки установки.
// Редактировать — в исходниках, Changelog/changelog.json.
//
// Номер версии в файле писать не нужно, он считается автоматически по SemVer, по СМЫСЛУ текста
// изменений, а не по формальному type: "Добавлена кнопка" — Patch, "Добавлен полноэкранный режим" —
// уже Minor, хотя type у обоих "added" (классификация — ChangeLevelClassifier, расчёт — BumpForChanges).
// Несовместимое изменение → major, крупная новая возможность → minor, иначе → patch. Несколько
// крупных пунктов в одной записи всё равно поднимают версию только один раз. База отсчёта — "1.0.0".
//
// Поле "date" необязательное: старые записи датированы и остаются такими, новые — нет (changelog
// теперь привязан к версии, а не к дате). Поэтому сортировка "по хронологии" считается по номеру
// версии, а не по дате напрямую — для старых записей это то же самое (версия и растёт по хронологии
// дат), для новых без даты только так порядок и определить.
//
// Порядок вычисления: сначала все записи с датой (от старой к новой), затем все без даты
// (в порядке, в котором перечислены в файле). "Текущая версия" — всегда последняя в этом порядке.
//
// Формат — простой JSON-массив, у каждого изменения свой type (added/changed/fixed/removed,
// неизвестный трактуется как changed), опциональное поле "image" у версии и у пункта — ссылка
// либо файл рядом с changelog.json:
// [
