...
internal static class ChangelogTranslationCatalog
{
    private static readonly IReadOnlyDictionary<string, string> EnglishByRussian =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            // Release 1.19.0
            ["Добавлена настройка показа действий контекстного меню трека: «Играть следующим», «Добавить в очередь», «Найти файл», избранное, операции копирования, свойства, теги и другие необязательные действия"] = "Added a setting to show or hide track context-menu actions: Play Next, Add to Queue, Find File, Favorites, copy actions, properties, tags, and other optional actions",
            ["Добавлены настраиваемые горячие клавиши для избранного, текста песни и переключения мини-плеера"] = "Added configurable hotkeys for Favorites, lyrics, and toggling the mini-player",
            ["Добавлена кнопка «Копировать аудиодиагностику»: локальный отчёт содержит данные WASAPI и не включает путь, название или исполнителя текущего трека"] = "Added a Copy audio diagnostics button: the local report contains WASAPI data and excludes the current track's path, title, and artist",
            ["Добавлен включаемый подробный trace подготовки трека: ReplayGain, теги, встроенная обложка и AudioFileReader; trace не содержит данных о треке или сетевых запросах"] = "Added an opt-in detailed track-preparation trace: ReplayGain, tags, embedded artwork, and AudioFileReader; the trace contains no track data or network requests",
            ["Блок «Диагностика Velopack» теперь свёрнут по умолчанию"] = "The Velopack Diagnostics section is now collapsed by default",
            ["Недоступная кнопка подготовки базового пакета отображается приглушённой и остаётся недоступной; после подготовки она показывает состояние «Базовый пакет подготовлен» / «Base package ready»"] = "The unavailable base-package preparation button appears dimmed and remains disabled; after preparation it shows Base package ready",
            ["Когда главное окно скрыто в режиме мини-плеера, окно настроек использует отдельную иконку настроек на панели задач"] = "When the main window is hidden in mini-player mode, the Settings window uses a separate Settings icon on the taskbar",
            ["Аудиодвижок переведён с NAudio 2.2.1 на NAudio 3.0.1 и WasapiPlayer в режиме WASAPI Shared"] = "Audio engine migrated from NAudio 2.2.1 to NAudio 3.0.1 and WasapiPlayer in WASAPI Shared mode",
            ["Анимация обложки при быстром переключении треков стала плавнее и не пропадает при серии быстрых переходов"] = "Album-art animation during rapid track switching is smoother and remains visible during a series of fast transitions",
            ["Карточка «Фактическое устройство» теперь показывает используемый формат вывода, запрошенную и фактическую задержку, состояние воспроизведения, активный Endpoint ID, маршрут устройства и причину последнего безопасного восстановления"] = "The Actual device card now shows the active output format, requested and actual latency, playback state, active Endpoint ID, device routing, and the reason for the last safe recovery",
            ["WPF-UI обновлён с 3.0.5 до 4.3.0"] = "WPF-UI updated from 3.0.5 to 4.3.0",
            ["Следующий трек подготавливается параллельно с защищённым fade-out/drain при переключении; защита от щелчков сохранена"] = "The next track is prepared in parallel with protected fade-out/drain during switching; click protection is preserved",
            ["Проект и GitHub CI переведены на .NET 10 LTS, xUnit v3 и Microsoft Testing Platform"] = "Project and GitHub CI migrated to .NET 10 LTS, xUnit v3, and Microsoft Testing Platform",
            ["Точки-ориентиры на подходящих слайдерах настроек выровнены с полосой и ключевыми значениями"] = "Milestone dots on applicable settings sliders are aligned with the track and key values",
