...
public partial class MainWindow : FluentWindow
{
    ...
    // Раньше здесь ещё вызывался WarmUpMainWindowLayout(): пока плеер стартует в мини-режиме,
    // обычное окно ни разу не показывается (см. StartupPresent), а раз оно ни разу не
    // Show()-нулось, WPF откладывает ВЕСЬ первый layout этого окна (Loaded, построение
    // контейнеров плейлиста — карточек папок, вложенных ListView, у каждого трека — свои
    // конвертеры и DataTrigger'ы) на самый первый настоящий Show(). Идея была в том, чтобы
    // прогнать этот первый Show()+layout заранее, в фоне на старте — тогда клик "Развернуть
    // плеер" из мини-режима позже ощущался бы мгновенным.
    //
    // На практике для окна с отключённой виртуализацией вложенных списков (см. комментарий
    // у PlaylistScrollViewer в MainWindow.xaml) это просто переносило ту же самую тяжёлую
    // синхронную работу с "клика Развернуть" на "самый старт" — WarmUpMainWindowLayout вызывал
    // Show()+UpdateLayout() ОДНИМ большим блокирующим проходом, без единого Dispatcher.Yield
    // внутри (UpdateLayout вообще не умеет уступать поток по частям, это атомарный вызов) — то
    // есть ровно то зависание, которое до этого убирали через постепенное добавление папок в
    // RestoreSavedPlaylistAsync, здесь же сводилось на нет одним махом. Раз в разговоре плеер
    // тестировался в основном именно в мини-режиме, пользователь каждый раз видел это
    // зависание прямо на старте, хотя по факту оно относилось не к загрузке плейлиста, а к
    // прогреву окна, которое даже не показывается.
    //
    // Убрали прогрев совсем — старт остаётся отзывчивым при любом стартовом виде, а первый
    // клик "Развернуть плеер" из мини-режима на очень большой библиотеке теперь может занять
    // заметное время (как и было до появления этого прогрева) — но это уже осознанный отклик
    // на явное действие пользователя, а не тишина сразу после запуска.
    ...
    // Кнопка "Проверить папку на новые треки" в заголовке группы — единственный способ
    // подхватить файлы, добавленные в папку на диске уже ПОСЛЕ того, как она попала в плейлист
    // (докинули mp3, пока плеер был закрыт, и т.п.). Раньше это ДОПОЛНИТЕЛЬНО делалось тихо и
    // безусловно на каждом старте (полный рекурсивный обход папки, SearchOption.AllDirectories,
    // для каждой папки плейлиста) — на большой или сетевой библиотеке это ощущалось как
    // "плейлист вроде появился мгновенно, но потом снова что-то долго сканируется в фоне на
    // каждом запуске, хотя ничего не менялось". Раз ручная кнопка для этого уже есть, тихий
    // авто-прогон при каждом старте убран целиком — обновлённый список папки восстанавливается
    // из settings.json как есть, без похода на диск, и остаётся ровно таким же, каким был
    // сохранён на выходе, пока пользователь сам не попросит проверить.
    //
    // Переиспользует AddFolderPath: он и так умеет, если папка с таким SourcePath уже есть в
    // плейлисте, добавить в неё только те файлы, которых там ещё нет — то есть делает ровно то
    // же самое, что нужно здесь, просто в ответ на клик. Выполняется синхронно на UI-потоке
    // (это единичное явное действие пользователя над одной папкой, а не фоновая проверка всего
    // плейлиста при старте) — как и у обычного "Добавить папку" из меню.
