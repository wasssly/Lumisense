...
public partial class MainWindow : FluentWindow
{
    ...
    public void StartupPresent()
    {
        ...
        {
            // Ничего не показываем — только значок в трее. RestorePlayerViewMode() чуть выше
            // уже мог сам создать и показать MiniPlayerWindow (если последним видом при
            // закрытии был мини-плеер — см. EnterMiniMode, он вызывает Show() безусловно, не
            // зная про эту настройку) — прячем его тоже. Обычное окно (this) само по себе
            // Show() ниже просто не получает, так что для обычного/квадратного/прямоугольного
            // вида прятать отдельно нечего.
            //
            // И Show(), и следующий сразу за ним Hide() (если понадобился) происходят здесь
            // синхронно, до того как управление вообще вернётся в цикл обработки сообщений —
            // тот же принцип, что и у StartupPresent в целом (см. комментарий выше): реального
            // видимого окна на экране пользователь увидеть не успевает.
            _miniPlayerWindow?.Hide();

            // Если стартовым видом оказался мини-плеер, EnterMiniMode() (вызванный чуть выше
            // из RestorePlayerViewMode) уже сам показал значок в трее — с более информативной
            // подсказкой ("Lumisense — Название трека"), чем просто "Lumisense". Не перезатираем
            // её здесь ещё раз.
    ...
    // Восстанавливает плейлист, сохранённый при прошлом закрытии, а также последний
    // проигранный трек и позицию в нём (без автозапуска). Удалённые/перемещённые с тех пор
    // файлы тихо убираются из списка уже после показа плейлиста (VerifyTrackExistenceInBackgroundAsync),
    // а не отфильтровываются до показа.
    //
    // Раньше это делалось синхронно в конструкторе, до появления окна на экране: File.Exists
    // по каждому треку на плейлисте в сотни-тысячи файлов, особенно на HDD/сетевом пути, суммарно
    // мог занимать секунды — плюс следом ещё LoadAndPlay последнего трека. Это и было основной
    // причиной долгого "чёрного экрана" при запуске. Уводили File.Exists в фоновый поток — но
    // список всё ещё ждал результата проверки, прежде чем появиться на экране.
    //
    // Теперь плейлист восстанавливается из settings.json как есть, без единого обращения к диску —
    // появляется в UI сразу. Устаревшие записи тихо исчезают чуть позже, когда их обнаружит
    // фоновая проверка — без цены задержки самого появления плейлиста.
    private async System.Threading.Tasks.Task RestoreSavedPlaylistAsync()
    {
        ...
        // нужна была только потому, что у списка не было настоящей виртуализации: контейнер для
        // каждой строки создавался в момент добавления трека, независимо от видимости на экране.
        //
        // Теперь PlaylistFoldersControl — настоящий виртуализирующий ListView: контейнеры
        // реализуются только для реально видимых строк, реассайн ItemsSource дешёвый независимо
        // от размера плейлиста — порционная догрузка больше не нужна, подготовка данных — это
        // чистая работа с
    ...
    // Кнопка "Проверить папку на новые треки" — единственный способ подхватить файлы,
    // добавленные в папку на диске уже после того, как она попала в плейлист. Раньше это
    // дополнительно делалось тихо на каждом старте (полный рекурсивный обход каждой папки
    // плейлиста) — на большой или сетевой библиотеке ощущалось как "плейлист появился, но
    // потом что-то ещё долго сканируется в фоне", хотя на диске ничего не менялось. Раз
    // ручная кнопка уже есть, тихий авто-прогон при старте убран целиком — папка
    // восстанавливается из settings.json как есть, без похода на диск.
    //
    // Переиспользует AddFolderPath: он и так умеет, если папка с таким SourcePath уже есть
    // в плейлисте, добавить в неё только недостающие файлы. Выполняется синхронно на UI-потоке —
    // это единичное действие пользователя над одной папкой, а не фоновая проверка при старте.
    ...
    private void OutputDevice_PlaybackStopped(object? sender, StoppedEventArgs e)
    {
        ...
        //
        // Раньше была проверка "_audioFile.Position >= _audioFile.Length - 1" — сравнение
        // в байтах с запасом всего в 1 байт. Из-за выравнивания по блокам сэмплов реальная
        // позиция на естественной остановке почти всегда на пару байт меньше Length, поэтому
        // условие часто было ложным — трек доигрывался, но плеер молча замолкал вместо
        // перехода к следующему (особенно заметно с "Повтором всего плейлиста"). Сравнение
        // по времени с секундным запасом устойчиво к этому округлению.
        Dispatcher.Invoke(() =>
        {
    ...
    // только из OnClosed — а поскольку MinimizeToTrayOnClose включён по умолчанию, обычное
    // закрытие крестиком почти всегда просто прячет окно в трей, и до настоящего "Выход" дело
    // может не доходить месяцами: settings.json оставался с треком от самого первого закрытия,
    // будто плеер вообще не запоминал последний. Теперь дополнительно вызывается при
    // сворачивании в трей, на паузе и периодически во время игры — актуальное состояние
    // почти всегда уже на диске к моменту следующего запуска, даже если процесс завершится
    // не штатно.
