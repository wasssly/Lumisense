...
public class AppSettings
{
    ...
    // рабочий стол) — "Mica" (по умолчанию: лёгкое размытие, почти непрозрачно) или "Acrylic"
    // (заметно сильнее размытие и полупрозрачность). Оба — современный системный backdrop
    // Windows 11 (DWM API, через Wpf.Ui.Controls.WindowBackdropType). Применяется только к
    // трём "основным" окнам приложения — MainWindow, SettingsWindow, StatisticsWindow (см. их
    // конструкторы/ApplyWindowBackdrop/OnSourceInitialized) — остальные мелкие диалоговые окна
    ...
    // это порядок добавления в избранное (см. FavoritesManager.GetOrder) — а НЕ порядок показа
    // на экране: закреплённые треки (см. PinnedFavoriteTracks ниже) при показе поднимаются
    // наверх независимо от того, когда их добавили, но сам этот список остаётся
    // неизменным порядком добавления.
    public List<string> FavoriteTracks { get; set; } = new();

    // Подмножество FavoriteTracks выше — какие из избранных треков закреплены наверху списка
    // "Избранное" (см. FavoritesManager.TogglePin/GetAll). Отдельный список, а не флаг на
    // самом треке — треки здесь всего лишь пути к файлам, у них просто нет места для
    ...
    // умолчанию включено — так мини-плеер вёл себя всегда, до появления этой настройки;
    // MiniPlayerPinned выше при этом сильнее — если положение закреплено, перетаскивания нет
    // вообще и прилипать уже нечему.
    //
    // У обычного окна плеера (MainWindow) такой настройки больше нет — перетаскивание там идёт
    // через системный ui:TitleBar (HTCAPTION), и попытка примагничивать его к краям экрана
    // (через LocationChanged, см. историю этого файла) на практике оказалась ненадёжной —
    // поэтому эту возможность для обычного окна убрали, а не оставили заведомо
    // не работающей. У мини-плеера перетаскивание устроено иначе (собственный DragMove из
    // MouseLeftButtonDown, см. MiniPlayerWindow) и WM_MOVING для него отрабатывает штатно.
