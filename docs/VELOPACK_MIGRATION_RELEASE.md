...
6. Только затем, при желании, удаляет старую Inno Setup-копию через **Windows → Installed apps**. Нельзя удалять `%AppData%\Lumisense` в диалоге удаления, если пользователь хочет сохранить данные.
